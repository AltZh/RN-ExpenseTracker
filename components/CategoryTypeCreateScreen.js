...
const StackScreen = ({ navigation }) => {
  ...
  const [isCounted, setIsCounted] = useState('1');

  const addToAccount = () => {
    db.transaction((tx) => {
      if(type == 'expense'){
        tx.executeSql(
          'INSERT INTO ExpenseCategoryType (name, is_counted) VALUES (?,?)',
          [name, isCounted],
          (tx, results) => {
      ...
      if(type == 'income'){
        tx.executeSql(
          'INSERT INTO IncomeCategoryType (name, is_counted) VALUES (?,?)',
          [name, isCounted],
          (tx, results) => {
            navigation.navigate('Categories List')
          }

...
const StackScreen = ({ navigation, route }) => {
  ...
  const [day, setDay] = useState(item.day || 0);
  const [month, setMonth] = useState(item.month || 0);
  const [year, setYear] = useState(item.year || 0);
  const [amountOld, _] = useState(Number(item.amount) || 0);
  const [amount, setAmount] = useState(String(item.amount) || '');
  const [category, setCategory] = useState(String(item.category) || '1');
  ...
  const updateItem = () => {
    db.transaction((tx) => {
      tx.executeSql(
        'UPDATE Expenses set amount=?, category=?, comment=?, day=?, month=?, year=? where id=?',
        [amount, category, comment, day, month, year, item.id],
        (tx, results) => {
          //navigation.navigate('Expenses List')
        }
      );

      if(Number(amount)>amountOld){
        let amountToMinus = (Number(amount) - amountOld);
        tx.executeSql(
          'UPDATE AccountBalance SET amount=(amount-'+amountToMinus+')',
          [],
          (tx, results) => {
            navigation.navigate('Expenses List')
            error=>console.log(error)
        ...
        let amountToAdd = (amountOld - Number(amount));
        tx.executeSql(
          'UPDATE AccountBalance SET amount=(amount+'+amountToAdd+')',
          [],
          (tx, results) => {
            navigation.navigate('Expenses List')
            error=>console.log(error)
      ...
      <View style={{ flex: 1, padding: 8 }}>
        ...
        <TouchableOpacity style={{ padding: 8 }}
          ...
          <View style={{ paddingVertical: 8, paddingHorizontal: 8, flexDirection: 'row', alignItems: 'center'}}>
            ...
            (<>
              <View style={{ 
                borderWidth: 0,  backgroundColor: category != 0 ? '#'+ categories[category].color : '#eee', 
                height: 16, width: 16, borderRadius: 16, marginRight: 8 }}></View>
              <Text style={{ fontSize: 18, color: '#333'  }}>
                { category != 0 ? categories[category].name : 'Не выбрана категория'}
              </Text>
            </>) : null }
          </View>
        </TouchableOpacity>
